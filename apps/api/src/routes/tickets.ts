...
r.get("/", (c) => {
  ...
  const rows = all<{
    ...
    ticket_name: string; session_label: string | null; qr: string; status: string; issued_at: string;
  }>(
  ...
  return ok(c, rows.map((t) => ({
    ...
    qr: t.qr, status: t.status, purchasedAt: Date.parse(t.issued_at + "Z") || Date.now(),
  })));
});
