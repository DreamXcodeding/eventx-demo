...
const genOrderNo = async (): Promise<string> => {
  for (let i = 0; i < 5; i++) {
    ...
    if (!(await get("select 1 as x from orders where order_no = ?", no))) return no;
  }
...
r.post("/", async (c) => {
  ...
  const ev = await get<{ id: string; title: string }>("select id, title from events where slug = ? limit 1", body.eventSlug);
  if (!ev) throw new ApiError(404, "NOT_FOUND", "ไม่พบอีเวนต์");

  const sessionLabel = body.items[0]?.sessionId
    ? (await get<{ date_label: string }>("select date_label from event_sessions where event_id = ? and code = ?", ev.id, body.items[0].sessionId))?.date_label ?? null
    : null;

  // ตรวจ ticket type + คำนวณยอดจากราคาจริงใน DB (กันลูกค้าส่งราคาปลอม)
  const lines: { tt: TT; quantity: number }[] = [];
  for (const it of body.items) {
    const tt = await get<TT>("select id, name, price from ticket_types where event_id = ? and code = ?", ev.id, it.ticketTypeId);
    if (!tt) throw new ApiError(400, "VALIDATION_ERROR", `ไม่พบประเภทบัตร ${it.ticketTypeId}`);
    lines.push({ tt, quantity: it.quantity });
  }
  const subtotal = lines.reduce((s, l) => s + l.tt.price * l.quantity, 0);

  const affiliateCode = body.affiliateCode?.trim() || null;
  const channel = affiliateCode && (await get("select 1 as x from affiliates where code = ?", affiliateCode)) ? "AFFILIATE" : "DIRECT";
...
r.post("/:orderNo/pay", async (c) => {
  ...
  const order = await get<{ id: string; user_id: string; status: string; subtotal: number; affiliate_code: string | null; expired: boolean }>(
    `select id, user_id, status, subtotal, affiliate_code, (now() > expires_at) as expired from orders where order_no = ? limit 1`, orderNo);
  ...
  const items = await all<{ event_id: string; event_title: string; ticket_name: string; session_label: string | null; quantity: number }>(
  ...
  for (const it of items) {
    const img = (await get<{ image: string | null }>("select image from events where id = ?", it.event_id))?.image ?? null;
  ...
  // commission: คำนวณก่อนเข้า transaction
  const aff = order.affiliate_code
...
r.get("/:orderNo", async (c) => {
  const user = c.get("user");
  const order = await get<{ id: string; user_id: string; order_no: string; status: string; subtotal: number; affiliate_code: string | null }>(
  ...
  const items = await all("select event_title, ticket_name, session_label, unit_price, quantity from order_items where order_id = ?", order.id);
