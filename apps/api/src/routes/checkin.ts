...
r.post("/", async (c) => {
  ...
  const t = get<{ id: string; status: string; event_title: string; ticket_name: string }>(
  ...
  run("update tickets set status = 'CHECKED_IN', checked_in_at = datetime('now') where id = ?", t.id);
