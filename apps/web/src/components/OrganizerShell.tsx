...
const NAV = [
  ...
  { to: "/organizer/events", key: "organizer.events" },
];
      ...
      <header className="sticky top-0 z-50 bg-navy-hero">
        <div className="mx-auto flex h-16 max-w-[1100px] items-center gap-6 px-6">
          <Link to="/organizer" className="flex items-center gap-2 text-white">
            ...
            <span className="rounded bg-white/15 px-2 py-0.5 text-[11px] font-medium text-white">ORGANIZER</span>
          </Link>
          <nav className="flex items-center gap-1">
            {NAV.map((n) => {
              const on = n.to === "/organizer" ? pathname === "/organizer" : pathname.startsWith(n.to);
              return (
          ...
          <div className="ml-auto flex items-center gap-2">
            <div className="hidden text-right sm:block">
              <p className="text-[13px] font-medium leading-tight text-white">{ORGANIZER.company}</p>
              <p className="text-[11px] leading-tight text-white/60">{t("organizer.feeLabel")} {(ORGANIZER.feeBps / 100).toFixed(0)}%</p>
            </div>
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-brand text-sm font-semibold text-white">{ORGANIZER.company.charAt(0)}</div>
          </div>
        </div>
      </header>
