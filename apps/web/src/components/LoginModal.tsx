...
export default function LoginModal() {
  ...
  useEffect(() => {
    if (open) {
      setMode(initialMode);
      setStep("form");
      setPhone(""); setOtp(""); setName(""); setEmail(""); setTouched(false);
    }
  }, [open, initialMode]);
  ...
  const finish = (user: { name: string; email: string; phone?: string }) => {
    ...
    login("demo-token", { id: "u-demo", name: user.name, email: user.email, phone: user.phone, role: isAffiliate ? "AFFILIATE" : "CUSTOMER" });
      ...
      <div role="dialog" aria-modal="true" aria-label={t("login.title")} className="ecn-rise relative z-10 w-full max-w-sm rounded-2xl bg-white p-7 shadow-e3" style={{ animationDuration: "0.18s" }}>
        ...
        {step === "otp" ? (
          <div className="mt-6">
            ...
            <button
              onClick={() => finish({ name: name.trim() || "คุณลูกค้า", email: `${phone}@eventx.demo`, phone })}
              disabled={!otpOk}
          ...
          <>
            ...
            {mode === "register" && (
              <div className="mt-5 space-y-3">
                ...
                <button
                  onClick={() => (regOk ? finish({ name: name.trim(), email: email.trim(), phone: phone.trim() }) : setTouched(true))}
                  className="w-full rounded-full bg-brand-gradient py-3.5 text-sm font-semibold text-white shadow-brand transition-all hover:opacity-95 active:scale-[0.98]"
                >
                  {t("login.regBtn")}
                </button>
              </div>
            ...
            <button onClick={() => finish({ name: "ผู้ใช้ Google", email: "demo@gmail.com" })} className="mb-3 flex w-full items-center justify-center gap-3 rounded-full border border-line py-3 text-sm font-medium text-ink transition-colors hover:bg-surface">
            ...
            <button onClick={() => finish({ name: "ผู้ใช้อีเมล", email: "demo@example.com" })} className="flex w-full items-center justify-center gap-3 rounded-full border border-line py-3 text-sm font-medium text-ink transition-colors hover:bg-surface">
