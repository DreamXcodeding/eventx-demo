...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  const displayName = user?.name ?? t("admin.role");
  return (
        ...
        <div className="mx-auto flex h-16 max-w-[1100px] items-center gap-6 px-6">
          ...
          <div className="relative ml-auto">
            {isAuth ? (
              ...
              <button onClick={() => openLogin("/admin")} className="rounded-md bg-white/15 px-4 py-1.5 text-sm font-medium text-white transition-colors hover:bg-white/25">{t("header.login")}</button>
            )}
          </div>
        </div>
      </header>
