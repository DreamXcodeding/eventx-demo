...
const ROLE_TK: Partial<Record<string, string>> = {
  CUSTOMER: "adminUsers.customer", AGENT: "adminUsers.agent", AFFILIATE: "adminUsers.affiliate", ORGANIZER: "adminUsers.organizer",
};
const ROLE_CLS: Partial<Record<string, string>> = {
  CUSTOMER: "bg-brand/10 text-brand", AGENT: "bg-[#fff4d6] text-[#8a6500]", AFFILIATE: "bg-success/10 text-success", ORGANIZER: "bg-navy/10 text-navy",
};
...
export default function AdminUsers() {
  ...
  const { data: apiUsers } = useApi<AdminUserDto[]>(() => api.admin.users(), []);
  const users: DirUser[] = USE_MOCK
    ? USERS
    : apiUsers.map((u) => ({
        id: u.id, name: u.name || "—", email: u.email, phone: u.phone,
        role: u.role as UserRole, joined: new Date(u.createdAt).toLocaleDateString("th-TH"),
        orders: 0, status: "ACTIVE" as const,
      }));

  const roleLabel = (r: string) => (ROLE_TK[r] ? t(ROLE_TK[r]!) : r);
  const roleCls = (r: string) => ROLE_CLS[r] ?? "bg-brand/10 text-brand";
                ...
                {list.map((u) => (
                  ...
                  >
                    ...
                    <td className="hidden px-4 py-3 sm:table-cell">
                      <span className={`rounded-full px-2.5 py-1 text-[11px] font-medium ${roleCls(u.role)}`}>{roleLabel(u.role)}</span>
                    </td>
              ...
              <>
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${roleCls(selected.role)}`}>{roleLabel(selected.role)}</span>
                  </div>
                </div>
