...
export default function AdminEvents() {
  ...
  const created = useAdminStore((s) => s.events);
  const addEvent = useAdminStore((s) => s.addEvent);
  const publishEvent = useAdminStore((s) => s.publishEvent);
  ...
  // รายการรวม: catalog (เผยแพร่แล้ว) + ที่แอดมินสร้าง
  ...
  const mine = created.map((e) => ({ id: e.id, title: e.title, province: e.province, category: e.category, dateLabel: e.dateLabel, priceFrom: e.priceFrom, status: e.status, created: true }));
              ...
              <tr key={e.id} className="border-b border-line last:border-0">
                ...
                <td className="px-4 py-3 text-right">
                  {e.status === "PUBLISHED" ? (
                    ...
                    <button onClick={() => e.created && publishEvent(e.id)} className="rounded-full bg-warning/15 px-2.5 py-1 text-[11px] font-medium text-[#8a6500] hover:bg-warning/25">{t("admin.draftPublish")}</button>
                  )}
                </td>
              </tr>
            ))}
